...
Execute "$exe_name" \
    -v=${verbosity_level} -s="${simulation_id}" -d=0 -testid=central_coex_test -RealEncryption=1

Execute "$exe_name" \
    -v=${verbosity_level} -s="${simulation_id}" -d=1 -testid=peripheral_coex_test -RealEncryption=1
